...
file_l = "/home/silvea/Downloads/stratum_ext.mp3"
# right file chanel
file_r = "/home/silvea/Downloads/stratum_ext.mp3"
...
value_x = 64         # value of cubes on X axis (must be over 0!)
value_y = 1         # value of cubes on Y axis (must be over 0!)
space_x = 2         # space between objects in X axis (orgins)
space_y = 2         # space betweeb objects in Y axis (orgins)
space_array = 1.3   # value of "Relative Offset" in array modifier, Z axis
scale = [1,1,0.5] # Dimensions of the cube (default object)
...
drivmod = 100.0      # driver "amplifier" (Imprecise column height)
...
if stereo == 2 or stereo == 3:
    matR = bpy.data.materials.new('visualisationMatR')
    ...
    for ry in range(0, value_y):
        for rx in range(0, value_x):
            ...
            bpy.context.object.data.materials.append(matR)
            cursor = bpy.context.scene.cursor_location[:]
            bpy.context.scene.cursor_location = (float_rx + space_x/2, float_ry, -scale[2])
...
if stereo == 1 or stereo == 3:
    matL = bpy.data.materials.new('visualisationMatL')
    float_rx = 0 + center_space
    float_ry = 0
    step = 0
    start = s
